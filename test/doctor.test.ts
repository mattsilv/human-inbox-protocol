...
import { buildPlist, plistPath, LaunchdManager } from "../src/daemon/launchd.js";
...
describe("bindRealityChecks (U5)", () => {
  ...
  beforeEach(() => {
    ...
    install({ manager: ld }); // loopback install, writes config + plist into fakeHome
  });
  afterEach(() => {
  ...
  it("reports no host/bind issues for a default loopback install", () => {
    ...
    const codes = bindRealityChecks(ld).map((i) => i.code);
  ...
  it("warns (not errors) on a scoped non-loopback bind; ok stays true", async () => {
    ...
    const issues = bindRealityChecks(ld);
  ...
  it("does not false-positive host-mismatch on equivalent IPv6 spellings (canonical compare)", async () => {
    ...
    expect(bindRealityChecks(ld).find((i) => i.code === "host-mismatch")).toBeUndefined();
  });
  ...
  it("errors on a plist↔config host mismatch", () => {
    ...
    const mismatch = bindRealityChecks(ld).find((i) => i.code === "host-mismatch");
  ...
  it("errors on an all-interfaces bind (0.0.0.0)", () => {
    ...
    const issue = bindRealityChecks(ld).find((i) => i.code === "bind-all-interfaces");
  ...
  it("errors when the LaunchAgent node binary is missing (Homebrew-GC class of bug)", () => {
    ...
    const issue = bindRealityChecks(ld).find((i) => i.code === "launchd-node-missing");
    expect(issue?.severity).toBe("error");
  });
